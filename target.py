...
class Vehicle:
    """
    Represents a vehicle in the EVRPTW model.

    Attributes:
        idx (int): Unique identifier for the vehicle.
        travel_path (list): A list of node indices representing the vehicle's route.
        travel_distance (float): The total distance traveled by the vehicle.
        current_load (float): The current load of the vehicle.
        current_battery (float): The current battery level of the vehicle.
        current_time (float): The current time in the vehicle's schedule.
    """
    ...
    def __init__(self, idx, graph):
        ...
        Parameters:
            idx (int): Unique identifier for the vehicle.
            graph (EvrptwGraph): The graph object containing problem data.
        """
        self.idx = idx
        self.travel_path = [0]  # Start at the depot
        self.travel_distance = 0.0
        self.current_load = 0.0
        self.current_battery = graph.tank_capacity
        self.current_time = 0.0
        self.visited_customers = set()
        self.unvisited_customers = {
            node.idx for node in graph.nodes if node.is_customer()
        }